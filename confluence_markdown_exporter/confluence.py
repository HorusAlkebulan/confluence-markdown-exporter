...
class Page(Document):
    ...
    def markdown(self) -> str:
        ...
        # to avoid __getattr__ interference from MarkdownConverter
    ...
    class Converter(TableConverter, MarkdownConverter):
        ...
        def get_markdown(self) -> str:
            ...
            This method is used instead of a @property to avoid conflicts with
            MarkdownConverter.__getattr__ which can interfere with property access.
            """
        ...
        def convert_page_properties(
            self, el: BeautifulSoup, text: str, convert_as_inline: bool
        ) -> None:
            rows = [
        ...
        def convert_alert(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            ...
            blockquote = super().convert_blockquote(el, text, convert_as_inline)
            return f"\n> [!{alert_type}]{blockquote}"

        def convert_div(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            ...
            if el.has_attr("data-macro-name"):
                ...
                if macro_name in macro_handlers:
                    return macro_handlers[macro_name](el, text, convert_as_inline)
            ...
            for class_name, handler in class_handlers.items():
                if class_name in str(el.get("class", "")):
                    return handler(el, text, convert_as_inline)
        ...
        def convert_expand_container(
            ...
            content = (
                self.process_tag(content_element, convert_as_inline).strip() if content_element else ""
            )

            # Return as details element
            return f"\n<details>\n<summary>{summary_text}</summary>\n\n{content}\n\n</details>\n\n"

        def convert_span(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            if el.has_attr("data-macro-name"):
                if el["data-macro-name"] == "jira":
                    return self.convert_jira_issue(el, text, convert_as_inline)

            return text

        def convert_attachments(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            ...
            return (
                f"\n\n{self.convert_table(BeautifulSoup(html, 'html.parser'), text, convert_as_inline)}\n"
            )
        ...
        def convert_column_layout(
            ...
            return self.convert_table(BeautifulSoup(html, "html.parser"), text, convert_as_inline)

        def convert_jira_table(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            ...
            return self.process_tag(jira_tables[0], convert_as_inline)

        def convert_toc(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            ...
            return self.process_tag(tocs[0], convert_as_inline)

        def convert_hidden_content(
            self, el: BeautifulSoup, text: str, convert_as_inline: bool
        ) -> str:
            content = super().convert_p(el, text, convert_as_inline)
            return f"\n<!--{content}-->\n"

        def convert_jira_issue(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            issue_key = el.get("data-jira-key")
            link = cast("BeautifulSoup", el.find("a", {"class": "jira-issue-key"}))
            if not link:
                return text
            if not issue_key:
                return self.process_tag(link, convert_as_inline)
        ...
        def convert_pre(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            if not text:
                return ""
        ...
        def convert_sub(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            return f"<sub>{text}</sub>"

        def convert_sup(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
        ...
        def convert_a(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:  # noqa: PLR0911
            if "user-mention" in str(el.get("class")):
                return self.convert_user_mention(el, text, convert_as_inline)
            if "createpage.action" in str(el.get("href")) or "createlink" in str(el.get("class")):
                if fallback := BeautifulSoup(self.page.editor2, "html.parser").find(
                    ...
                    return self.convert_a(fallback, text, convert_as_inline)  # type: ignore -
                return f"[[{text}]]"
            ...
            if "attachment" in str(el.get("data-linked-resource-type")):
                link = self.convert_attachment_link(el, text, convert_as_inline)
        ...
        def convert_time(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            if el.has_attr("datetime"):
                return f"{el['datetime']}"

            return f"{text}"

        def convert_user_mention(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
        ...
        def convert_li(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            md = super().convert_li(el, text, convert_as_inline)
        ...
        def convert_img(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            attachment = None
        ...
        def convert_table(self, el: BeautifulSoup, text: str, convert_as_inline: bool) -> str:
            if el.has_attr("class") and "metadata-summary-macro" in el["class"]:
                return self.convert_page_properties_report(el, text, convert_as_inline)
        ...
        def convert_page_properties_report(
            ...
            if not table:
                return ""
            return super().convert_table(table, "", convert_as_inline)  # type: ignore -
